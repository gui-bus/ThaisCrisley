...
- **DESIGN SYSTEM:**
  - **Layout:** The `body` tag in `layout.tsx` **MUST** always maintain the classes `mx-auto w-full max-w-440` for layout consistency. **NEVER** use the `container` or `max-w-7xl` classes; instead, use custom max-widths (like `max-w-6xl` or `max-w-screen-xl`) and responsive padding (`px-6 md:px-12 lg:px-24`) for precise control.
